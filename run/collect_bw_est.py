...
for BASE_BW_MUL in BASE_BW_MUL_list:
    ...
    bw_est_logfile = os.path.join(
        basedir, "threads", "worker{}".format(port), "bw_est_bw{} copy 2.log".format(bw_est_groudtruth))
    ...
    for i in range(COL_NUM):
        ...
        ax[i].set_title("500samples; {}ack".format(i+2))
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(basedir, "threads", "worker{}".format(
        port), "bw_est_bw{}_{}pc.png".format(bw_est_groudtruth, SLICE)))
